...
func newText(ctx context.Context, updateText <-chan string, errorCh chan<- error) (*text.Text, error) {
	...
	// listens on the given update channel for new data and updates widget with it
	go func() {
		for {
			select {
			case t := <-updateText:
	...
	go func() {
		for {
			...
			case t := <-updateText:
				...
				errorCh <- sd.Write([]*segmentdisplay.TextChunk{	//allows passing multiple text segments to display
